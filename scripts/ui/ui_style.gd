...
static func pop_in(c: Control, dir: int = Dir.DOWN, dist: float = 12.0, dur: float = 0.15) -> void:
	...
	tw.tween_property(c, "modulate:a", 1.0, dur).set_trans(Tween.TRANS_CUBIC).set_ease(
		Tween.EASE_OUT
	)
	...
	if not (c.get_parent() is Container):
		...
		match dir:
			Dir.DOWN:
				off = Vector2(0.0, -dist)
			Dir.UP:
				off = Vector2(0.0, dist)
			Dir.LEFT:
				off = Vector2(dist, 0.0)
			Dir.RIGHT:
				off = Vector2(-dist, 0.0)
		var base: Vector2 = c.position
		c.position = base + off
		tw.tween_property(c, "position", base, dur).set_trans(Tween.TRANS_CUBIC).set_ease(
			Tween.EASE_OUT
		)
