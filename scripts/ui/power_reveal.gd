...
func _process(delta: float) -> void:
	...
	if not _revealing:
		if (
			_player != null
			and _player.has_method("active_buffs")
...
func _draw_reveal(vp: Vector2) -> void:
	...
	draw_rect(
		Rect2(origin, Vector2(PANEL_W, PANEL_H)),
		Color(accent, 0.6 + (0.4 if landed else 0.0)),
		false,
		2.0
	)
	# Header.
	...
	if icon != null:
		...
		draw_texture_rect(
			icon,
			Rect2(cpos + Vector2(pad, pad), Vector2(cs - pad * 2.0, cs - pad * 2.0)),
			false,
			col
		)
	else:
		...
		draw_string(
			font,
			cpos + Vector2(cs * 0.5 - 9.0, cs * 0.5 + 11.0),
			initial,
			HORIZONTAL_ALIGNMENT_LEFT,
			-1,
			30,
	...
	draw_string(
		font,
		Vector2(name_x, origin.y + 58.0),
		String(def.get("name", _face)),
		HORIZONTAL_ALIGNMENT_LEFT,
		PANEL_W - CARD - 46.0,
		18,
...
func _draw_buff_strip(vp: Vector2) -> void:
	...
	for b in buffs:
		...
		if icon != null:
			draw_texture_rect(
				icon, Rect2(Vector2(x + 9.0, y + 3.0), Vector2(18.0, 18.0)), false, col
			)
		draw_string(
			font,
			Vector2(x + 32.0, y + 17.0),
			String(b.get("name", "")),
			HORIZONTAL_ALIGNMENT_LEFT,
			108.0,
			13,
