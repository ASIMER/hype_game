...
const ENEMY_BUILDERS := ["robot_tick", "robot_wasp", "robot_bastion", "robot_boss", "robot_caller",
	...
	"robot_frosthound", "robot_cryomortar", "robot_avalanche",
	"robot_oni", "robot_kappa", "robot_raiju"]
...
static func build(id: String) -> Node3D:
	match id:
		"robot_tick": return build_robot_tick()
		...
		"robot_frosthound": return build_robot_frosthound()
		"robot_cryomortar": return build_robot_cryomortar()
		"robot_avalanche": return build_robot_avalanche()
		"robot_oni": return build_robot_oni()
		"robot_kappa": return build_robot_kappa()
		"robot_raiju": return build_robot_raiju()
		...
		"loot_stim": return build_stim()
		"loot_circuit": return build_circuit(AssetRegistry.get_color(id), 1)
		"loot_circuit_pack": return build_circuit(AssetRegistry.get_color(id), 3)
		"loot_artifact": return build_artifact()
		"loot_grenade": return build_grenade(AssetRegistry.get_color(id), false)
		"loot_grenade_mk2": return build_grenade(AssetRegistry.get_color(id), true)
		"loot_ammo": return build_ammo()
		"loot_scrap": return build_scrap()
		"loot_plastic": return build_plastic()
		"loot_data_chip": return build_data_chip()
	if id in ATT_OPTIC: return build_att_optic(id)
	if id in ATT_MAG: return build_att_mag(id)
	if id in ATT_BARREL: return build_att_barrel(id)
	if id in ATT_GRIP: return build_att_grip(id)
	if id.begins_with("schematic_"): return build_schematic(AssetRegistry.get_color(id))
	return null
...
# ----------------------------------------------------------------- material
static func _mat(color: Color, metallic := 0.0, roughness := 0.7,
...
static func _part(parent: Node3D, mesh: Mesh, mat: StandardMaterial3D,
		offset := Vector3.ZERO, rot_deg := Vector3.ZERO, scale := Vector3.ONE) -> MeshInstance3D:
	...
	if rot_deg != Vector3.ZERO:
		basis = Basis.from_euler(Vector3(
			deg_to_rad(rot_deg.x), deg_to_rad(rot_deg.y), deg_to_rad(rot_deg.z))) * basis
...
static func _strut(parent: Node3D, a: Vector3, b: Vector3, radius: float,
		mat: StandardMaterial3D, seg := 8) -> MeshInstance3D:
	var diff := b - a
...
static func build_grenade(col: Color, mk2: bool) -> Node3D:
	...
	_part(root, _box(Vector3(0.03, 0.18, 0.05)), metal, Vector3(0.07, r + 0.06, 0), Vector3(0, 0, 18))  # lever
	_part(root, _cyl(0.035, 0.02, 10), metal, Vector3(0.1, r * 2 + 0.0, 0.0), Vector3(90, 0, 0))        # pin ring
	if mk2:
		_part(root, _cyl(r + 0.01, 0.04, 12), _mat(Color(0.4, 1.0, 0.5), 0, 0.3, Color(0.3, 1.0, 0.4), 3.0), Vector3(0, r + 0.02, 0))
	return root
...
static func build_scrap() -> Node3D:
	...
	_part(root, _box(Vector3(0.14, 0.1, 0.12)), m1, Vector3(-0.1, 0.07, -0.05), Vector3(-20, 35, 15))
...
static func build_plastic() -> Node3D:
	...
	for i in range(3):
		var c: Color = cols[i]
		_part(root, _box(Vector3(0.3, 0.05, 0.22)), _mat(c, 0.0, 0.6),
			Vector3((i - 1) * 0.04, 0.06 + i * 0.06, (i - 1) * 0.03), Vector3(0, i * 12, 0))
	return root
...
static func build_schematic(col: Color) -> Node3D:
	...
	for i in range(3):
		_part(root, _box(Vector3(0.3, 0.03, 0.012)), line, Vector3(0, 0.13 + i * 0.002, -0.07 + i * 0.07), Vector3(-12, 0, 0))
	_part(root, _box(Vector3(0.012, 0.03, 0.2)), line, Vector3(0.0, 0.135, 0), Vector3(-12, 0, 0))
	_part(root, _box(Vector3(0.08, 0.04, 0.06)), line, Vector3(0.08, 0.14, 0.02), Vector3(-12, 0, 0))
	return root
...
static func build_att_optic(id: String) -> Node3D:
	...
	_part(root, _cyl(tube_r + 0.01, 0.03, 14), lens, Vector3(0, 0.22, -tube_len * 0.5), Vector3(90, 0, 0))
...
static func build_att_mag(id: String) -> Node3D:
	...
	if id == "att_drum_mag":
		...
		_part(root, _box(Vector3(0.12, h, 0.09)), body, Vector3(0, h * 0.5 + 0.04, 0.0), Vector3(14, 0, 0))
...
static func build_att_grip(id: String) -> Node3D:
	...
	_part(root, _cyl(0.045, 0.28, 12), body, Vector3(0, 0.18, 0.02),
		Vector3(20, 0, 0) if angled else Vector3(2, 0, 0))                            # grip
...
static func build_robot_frosthound() -> Node3D:
	...
	var shell := ProcMaterials.weathered(Color(0.62, 0.7, 0.76), 0.55, 0.45, 0.5, 53, Vector3(0.6, 0.6, 0.6), false)
	...
	for i in 3:
		_part(root, _cone(0.07, 0.26 - float(i) * 0.05, 6), ice, Vector3(0, 0.92, -0.2 + float(i) * 0.3), Vector3(-12, 0, 0))
	...
	for side in [-1.0, 1.0]:
		for z in [-0.38, 0.42]:
			...
			_strut(pivot, Vector3(side * 0.06, -0.32, 0.06), Vector3(side * 0.08, -0.6, -0.02), 0.05, leg_mat)
...
static func build_robot_cryomortar() -> Node3D:
	...
	var shell := ProcMaterials.weathered(Color(0.45, 0.52, 0.6), 0.6, 0.45, 0.55, 59, Vector3(0.6, 0.6, 0.6), false)
...
static func build_robot_avalanche() -> Node3D:
	...
	var plate := ProcMaterials.weathered(Color(0.78, 0.83, 0.88), 0.5, 0.5, 0.45, 61, Vector3(0.6, 0.6, 0.6), false)
...
static func build_robot_oni() -> Node3D:
	...
	var armor := ProcMaterials.weathered(Color(0.26, 0.22, 0.24), 0.55, 0.5, 0.55, 67, Vector3(0.6, 0.6, 0.6), false)
	...
	for i in 4:
		...
		_part(root, _box(Vector3(0.26, 0.34, 0.06)), lacquer, Vector3(a, 0.78, -0.26), Vector3(8, 0, 0))
		_part(root, _box(Vector3(0.26, 0.34, 0.06)), lacquer, Vector3(a, 0.78, 0.26), Vector3(-8, 0, 0))
	...
	_part(root, _box(Vector3(0.05, 0.9, 0.14)), _mat(Color(0.75, 0.78, 0.85), 0.85, 0.2), Vector3(0.9, 0.65, -0.45), Vector3(30, 0, 0))
	# Left fist.
...
static func build_robot_kappa() -> Node3D:
	...
	var shell := ProcMaterials.weathered(Color(0.24, 0.36, 0.28), 0.5, 0.5, 0.55, 71, Vector3(0.6, 0.6, 0.6), false)
	...
	_part(root, _sphere(0.46, true, 8, 14), dark, Vector3(0, 0.78, 0.18), Vector3(-18, 0, 0), Vector3(1.0, 0.7, 1.1))
	for i in 3:
		_part(root, _cone(0.06, 0.14, 6), shell, Vector3(0, 1.06 - float(i) * 0.1, 0.1 + float(i) * 0.22), Vector3(-20, 0, 0))
...
static func build_robot_raiju() -> Node3D:
	...
	var body := ProcMaterials.weathered(Color(0.3, 0.34, 0.45), 0.6, 0.4, 0.5, 73, Vector3(0.6, 0.6, 0.6), false)
	...
	core.name = "Core"
	for i in 2:
		_part(root, _box(Vector3(0.04, 0.16, 0.2)), bolt, Vector3(0, 0.84, -0.05 + float(i) * 0.34), Vector3(-16, 0, 0))
	...
	for i in 2:
		...
		root.add_child(ant)
		_part(ant, _cone(0.03, 0.3, 5), bolt, Vector3(side * 0.04, 0.12, 0.02), Vector3(0, 0, -side * 18.0))
	# Bushy segmented tail (+Z) + 4 thin legs.
	_part(root, _cone(0.1, 0.5, 6), body, Vector3(0, 0.74, 0.72), Vector3(115, 0, 0))
	for side in [-1.0, 1.0]:
		for z in [-0.3, 0.38]:
			var zf := float(z)
			_strut(root, Vector3(side * 0.16, 0.55, zf), Vector3(side * 0.2, 0.0, zf + 0.04), 0.045, leg_mat)
	return root
