...
static func build(id: String) -> Node3D:
	match id:
		"smg": return _build_smg()
		"shotgun": return _build_shotgun()
		"pistol": return _build_pistol()
		"dmr": return _build_dmr()
	return null
...
static func _gunmetal() -> StandardMaterial3D:
	return ProcMaterials.weathered(Color(0.13, 0.14, 0.16), 0.6, 0.45, 0.55, 7,
		Vector3(0.4, 0.4, 0.4), false)
...
static func _barrel(parent: Node3D, r: float, length: float, z: float, y: float,
		mat: StandardMaterial3D, seg := 12) -> MeshInstance3D:
...
static func _build_pistol() -> Node3D:
	...
	_part(root, _box(Vector3(0.04, 0.03, 0.05)), metal, Vector3(0, -0.135, 0.085), Vector3(12, 0, 0))  # mag base
