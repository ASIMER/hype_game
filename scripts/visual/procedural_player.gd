...
const PAINTS := {
	"paint_raider":  { "primary": Color(0.30, 0.40, 0.52), "secondary": Color(0.18, 0.23, 0.30), "accent": Color(0.25, 0.80, 0.85) },
	"paint_ash":     { "primary": Color(0.42, 0.44, 0.47), "secondary": Color(0.22, 0.23, 0.25), "accent": Color(0.90, 0.55, 0.20) },
	"paint_crimson": { "primary": Color(0.55, 0.15, 0.15), "secondary": Color(0.16, 0.10, 0.10), "accent": Color(1.00, 0.55, 0.20) },
	"paint_forest":  { "primary": Color(0.30, 0.40, 0.22), "secondary": Color(0.20, 0.17, 0.10), "accent": Color(0.75, 0.85, 0.30) },
	"paint_arctic":  { "primary": Color(0.78, 0.84, 0.90), "secondary": Color(0.35, 0.42, 0.50), "accent": Color(0.35, 0.75, 1.00) },
	"paint_void":    { "primary": Color(0.13, 0.13, 0.17), "secondary": Color(0.07, 0.07, 0.09), "accent": Color(0.70, 0.35, 1.00) },
	"paint_toxic":   { "primary": Color(0.20, 0.26, 0.18), "secondary": Color(0.12, 0.14, 0.10), "accent": Color(0.55, 1.00, 0.25) },
...
static func defaults() -> Dictionary:
	return { "head": "head_dome", "torso": "torso_plated", "arms": "arms_standard",
		"legs": "legs_standard", "paint": "paint_raider" }
...
static func _mats(paint: Dictionary) -> Dictionary:
	...
	return {
		"armor": ProcMaterials.weathered(primary, 0.45, 0.55, 0.5, 7, Vector3(0.5, 0.5, 0.5), false),
...
static func build_part(category: String, variant_id: String, paint_id: String) -> Node3D:
	...
	match category:
		"head":  _build_head(root, variant_id, mats)
		"torso": _build_torso(root, variant_id, mats)
		"arms":  _build_arms(root, variant_id, mats)
		"legs":  _build_legs(root, variant_id, mats)
	return root

# Shorthand to keep the builders readable.
static func _p(root: Node3D, mesh: Mesh, mat: StandardMaterial3D, off: Vector3,
		rot := Vector3.ZERO, scl := Vector3.ONE) -> MeshInstance3D:
	return ProceduralModels._part(root, mesh, mat, off, rot, scl)
static func _box(s: Vector3) -> BoxMesh: return ProceduralModels._box(s)
static func _sph(r: float, hemi := false, ri := 8, ra := 12) -> SphereMesh: return ProceduralModels._sphere(r, hemi, ri, ra)
static func _cyl(r: float, h: float, seg := 10) -> CylinderMesh: return ProceduralModels._cyl(r, h, seg)
static func _cone(r: float, h: float, seg := 10) -> CylinderMesh: return ProceduralModels._cone(r, h, seg)
...
static func _build_legs(root: Node3D, variant: String, m: Dictionary) -> void:
	match variant:
		"legs_standard": _legs_standard(root, m)
		"legs_light":    _legs_light(root, m)
		"legs_heavy":    _legs_heavy(root, m)
		"legs_digi":     _legs_digi(root, m)
		"legs_boots":    _legs_boots(root, m)
		"legs_thruster": _legs_thruster(root, m)
		"legs_armored":  _legs_armored(root, m)
		"legs_runner":   _legs_runner(root, m)
		"legs_tank":     _legs_tank(root, m)
		"legs_greaves":  _legs_greaves(root, m)
		_: _legs_standard(root, m)
...
static func _build_torso(root: Node3D, variant: String, m: Dictionary) -> void:
	match variant:
		"torso_plated":  _torso_plated(root, m)
		"torso_light":   _torso_light(root, m)
		"torso_heavy":   _torso_heavy(root, m)
		"torso_rig":     _torso_rig(root, m)
		"torso_reactor": _torso_reactor(root, m)
		"torso_riot":    _torso_riot(root, m)
		"torso_recon":   _torso_recon(root, m)
		"torso_exo":     _torso_exo(root, m)
		"torso_trench":  _torso_trench(root, m)
		"torso_jugg":    _torso_jugg(root, m)
		_: _torso_plated(root, m)
...
static func _torso_plated(root: Node3D, m: Dictionary) -> void:
	...
	core.name = "Core"
	for sx: float in [-1.0, 1.0]:
		_p(root, _sph(0.15, true, 8, 10), m.armor, Vector3(sx * 0.3, 1.38, 0.0), Vector3.ZERO, Vector3(1.0, 0.7, 1.0))
...
static func _torso_reactor(root: Node3D, m: Dictionary) -> void:
	...
	var core := _p(root, _cyl(0.09, 0.08, 16), m.accent, Vector3(0.0, 1.2, -0.18), Vector3(90, 0, 0))
	core.name = "Core"
	for sx: float in [-1.0, 1.0]:
		_p(root, _sph(0.14, true, 8, 10), m.armor, Vector3(sx * 0.3, 1.38, 0.0), Vector3.ZERO, Vector3(1.0, 0.7, 1.0))
...
static func _torso_jugg(root: Node3D, m: Dictionary) -> void:
	...
	for sx: float in [-1.0, 1.0]:
		_p(root, _sph(0.20, true, 8, 10), m.armor, Vector3(sx * 0.40, 1.40, 0.0), Vector3.ZERO, Vector3(1.0, 0.8, 1.0))
...
static func _build_arms(root: Node3D, variant: String, m: Dictionary) -> void:
	match variant:
		"arms_standard": _arms_standard(root, m)
		"arms_light":    _arms_light(root, m)
		"arms_bulky":    _arms_bulky(root, m)
		"arms_servo":    _arms_servo(root, m)
		"arms_pauldron": _arms_pauldron(root, m)
		"arms_blade":    _arms_blade(root, m)
		"arms_gauntlet": _arms_gauntlet(root, m)
		"arms_piston":   _arms_piston(root, m)
		"arms_shield":   _arms_shield(root, m)
		"arms_claw":     _arms_claw(root, m)
		_: _arms_standard(root, m)
...
static func _arms_gauntlet(root: Node3D, m: Dictionary) -> void:
	for sx: float in [-1.0, 1.0]:
		...
		# knuckle bar ridges
		for i: int in range(3):
			_p(root, _box(Vector3(0.19, 0.035, 0.06)), m.trim, Vector3(x * 1.05, 0.90 - float(i) * 0.08, -0.09))
...
static func _arms_piston(root: Node3D, m: Dictionary) -> void:
	for sx: float in [-1.0, 1.0]:
		...
		_p(root, _cyl(0.03, 0.22, 8), m.metal, Vector3(x * 1.05 + sx * 0.03, 1.20, 0.04), Vector3(10, 0, 0))
		_p(root, _cyl(0.025, 0.14, 8), m.accent, Vector3(x * 1.05 + sx * 0.03, 1.10, 0.05), Vector3(10, 0, 0))
		_p(root, _cyl(0.03, 0.22, 8), m.metal, Vector3(x * 1.05 - sx * 0.03, 1.20, 0.04), Vector3(10, 0, 0))
		_p(root, _cyl(0.025, 0.14, 8), m.accent, Vector3(x * 1.05 - sx * 0.03, 1.10, 0.05), Vector3(10, 0, 0))
...
static func _arms_claw(root: Node3D, m: Dictionary) -> void:
	for sx: float in [-1.0, 1.0]:
		...
		_p(root, _cone(0.025, 0.11, 6), m.trim, Vector3(x * 1.05 + sx * 0.05, 0.65, -0.02), Vector3(-80, 0, sx * 15))
		_p(root, _cone(0.025, 0.11, 6), m.trim, Vector3(x * 1.05 - sx * 0.05, 0.65, -0.02), Vector3(-80, 0, sx * -15))
...
static func _build_head(root: Node3D, variant: String, m: Dictionary) -> void:
	...
	match variant:
		"head_dome":    _head_dome(root, m)
		"head_visor":   _head_visor(root, m)
		"head_antenna": _head_antenna(root, m)
		"head_cyclops": _head_cyclops(root, m)
		"head_horned":  _head_horned(root, m)
		"head_crest":   _head_crest(root, m)
		"head_hood":    _head_hood(root, m)
		"head_mohawk":  _head_mohawk(root, m)
		"head_riot":    _head_riot(root, m)
		"head_oni":     _head_oni(root, m)
		_: _head_dome(root, m)

# head_dome — smooth sphere helm, single wide visor slit.
static func _head_dome(root: Node3D, m: Dictionary) -> void:
	_p(root, _sph(0.17, false, 10, 12), m.armor, Vector3(0.0, 1.62, 0.0), Vector3.ZERO, Vector3(1.0, 1.1, 1.0))
...
static func _head_horned(root: Node3D, m: Dictionary) -> void:
	_p(root, _sph(0.17, false, 10, 12), m.armor, Vector3(0.0, 1.62, 0.0), Vector3.ZERO, Vector3(1.0, 1.05, 1.0))
	var eye := _p(root, _box(Vector3(0.26, 0.06, 0.04)), m.accent, Vector3(0.0, 1.62, -0.15))
	eye.name = "Eye"
	for sx: float in [-1.0, 1.0]:
		_p(root, _cone(0.045, 0.22), m.trim, Vector3(sx * 0.12, 1.78, 0.02), Vector3(-20, 0, sx * 30))
...
static func _head_hood(root: Node3D, m: Dictionary) -> void:
	...
	_p(root, _sph(0.2, true, 10, 12), m.armor, Vector3(0.0, 1.66, 0.04), Vector3(10, 0, 0), Vector3(1.1, 1.0, 1.2))
...
static func _head_riot(root: Node3D, m: Dictionary) -> void:
	...
	# visor cage bars
	for i: int in range(3):
		_p(root, _box(Vector3(0.30, 0.015, 0.05)), m.metal, Vector3(0.0, 1.57 + float(i) * 0.05, -0.16))
...
static func _head_oni(root: Node3D, m: Dictionary) -> void:
	...
	for sx: float in [-1.0, 1.0]:
		...
		# small brow spike
		_p(root, _cone(0.03, 0.10, 6), m.trim, Vector3(sx * 0.12, 1.77, -0.04), Vector3(-15, 0, sx * 20))
