...
## License posture
All SHIPPED assets are **CC0** (public domain, no attribution required) or **MIT** (attribution kept in this file + the addon's own LICENSE). **No proprietary-EULA audio (Ovani/Sonniss) is committed** — their licenses forbid exposing raw files in a public repo. Synty is paid (not used).
