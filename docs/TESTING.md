...
## §8. QA-ловушки харнесса (проверено кровью, 2026-08-11)

Каждая из этих ловушек стоила реальной сессии дебага — проверяй список ПЕРВЫМ при странном QA.

1. **`godmode` требует JSON-bool**: `{"cmd":"godmode","on":true}`. Строка `"on"` доходит до
   `bool("on")` в GDScript — это невалидная конверсия, хендлер молча умирает и верб
   ТАЙМАУТИТСЯ без ответа (бридж при этом жив — следующие команды работают). Симптом
   «какой-то верб перестал отвечать» = проверь типы значений в payload.
2. **`drivable` — ТОП-ЛЕВЕЛ ключ `state`**, не `state.player.drivable`. Луп ожидания
   готовности через `state.player` будет ждать вечно/до таймаута.
3. **Выживший QA-немезис охотится на каждом буте**: провальный QA-рейд рождает немезиса в
   `user://nemesis*.cfg` (`%APPDATA%\Godot\app_userdata\Hype Raiders\nemesis*.cfg` + пер-порт
   варианты). На следующем буте он форс-инжектится хантером и убивает AFK-игрока за ~15-40с —
   выглядит как «свежий матч мгновенно проигран» (phase 4, hp 30 = downed-пул). Чисти
   `nemesis*.cfg` перед QA-сессией.
4. **AFK-игрок = кемпер**: анти-кемп директора (DIRECTOR_CAMP_*) флангует стоящего игрока
   хантерами («Flanked!»). Скрипт, где игрок стоит между пробами >20-30с, будет умирать «сам
   по себе». Двигай игрока (`goto`/`tp`) между замерами. (Первые 60с спавн-грейса анти-кемп
   молчит — но QA обычно живёт дольше.)
5. **EnemyDance-джук ломает скриптовый aim+fire**: враги теперь уворачиваются, когда на них
   наводятся — «прицелился-выстрелил» промахивается почти всегда. Для киллов С АТРИБУЦИЕЙ
   игроку используй `hurt`-верб (`_debug_hurt` бьёт с source=локальный игрок → срабатывают
   `credit_kill`/`player_kill`/XP/квесты) или `quest grantkills` (эмитит только `player_kill`,
   квесты/левелап-бар — да, XP-попапы — нет).
6. **`restart` из RaidSummary работает**, но миксовать его с параллельными таймаут-ретраями
   опасно — два рестарта подряд могут словить гонку. Один restart → терпеливый wait-луп
   (top-level `drivable`) до 60с.
7. **Скриншот «гигантских стеклянных плит» на пол-экрана** — это конус extraction-маяка
   вплотную к камере (или DOF-блюр стены в упор), не баг UI.
8. **Прицел в упор (<3м) задирает камеру в небо** — известная ловушка `aim point`; целься
   в точку на ±1.0-1.6м высоты у дальних целей, для близких — бери точку дальше за целью.
9. **Новые функции в чужие файлы вставляй ТОЛЬКО между функциями** (не по анкору на коммент
   внутри тела): Edit-вставка в середину `_ready` режет функцию — хвост становится валидным
   мёртвым кодом, НОЛЬ ошибок в лог (так «опустел» весь хаб и ломался SettingsManager).
10. **Rest-трансформы модели захватывай ДО спавн-твинов** (`LimbBurst.assemble` ставит scale
    0.05 в кадре вызова — захват «после» заморозит rest на 0.05, и первый же флинч схлопнет
    модель в точку).
11. **Код-билт Control под CanvasLayer**: только `set_anchors_and_offsets_preset(FULL_RECT)` —
    plain `set_anchors_preset` оставляет рут 0×0, и все bottom-якорные дети рисуются за
    экраном сверху-слева (дебаг — печатай `size`/`global_position` виджета).
