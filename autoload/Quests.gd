...
func claim(id: String) -> bool:
	...
	if line != null:
		...
		ql_complete = (
			int(lp.get("total", 0)) > 0 and int(lp.get("done", 0)) >= int(lp.get("total", 0))
		)
	...
	(
		...
		. emit(
			id,
			{
				"currency": q.reward_currency,
				"items": q.reward_items(),
				"blueprints": Array(q.reward_blueprints),
				"xp": q.reward_xp,
				"rep": q.reward_rep,
				"skill_points": q.reward_skill_points,
				"giver": giver,
				"giver_rep": grep if giver != "" else 0,
				"cosmetics": unlocked_cos,
				"questline_complete": ql_complete,
	...
	if line != null:
		for sid in line.quest_ids:
			if is_claimed(String(sid)):
				...
				current = String(sid)
	return {
		"done": done, "total": line.quest_ids.size() if line != null else 0, "current_id": current
	}
...
func _stat(key: String) -> float:
	...
	match key:
		"mobs_total":
			return float(MetaProgression.total_mob_kills())
		"raider_level":
			return float(MetaProgression.raider_level)
		"vendor_rep":
			return float(MetaProgression.vendor_rep)
		"currency":
			return float(MetaProgression.currency)
		"xp":
			return float(MetaProgression.xp)
		"extractions":
			return float(MetaProgression.extractions_total)
	return NAN
...
func unlock_hint(q: QuestData) -> String:
	...
	return (
		tr("Unlock by: ") + ", ".join(parts)
		if not parts.is_empty()
		else tr("Unlock by progressing")
	)
...
func seconds_until_daily_reset() -> int:
	...
	var secs_into_day := (
		int(t.get("hour", 0)) * 3600 + int(t.get("minute", 0)) * 60 + int(t.get("second", 0))
	)
